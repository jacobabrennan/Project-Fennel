...
var spiderPrototype = (function (){
    return Object.create(enemy, {
        ...
        //skills: {value: ["web","attack"], writable: true},
        constructor: {value: function (options){
            enemy.constructor.apply(this, arguments);
            var self = this;
            gameManager.registerEvent(function (){
                var ownLevel = mapManager.getLevel(self.levelId);
                var webDist = 5;
                var webWidth = (webDist*2)+1;
                viewGrid = ownLevel.getView(self.x, self.y, webDist);
                for(var posY = -webDist; posY <= webDist; posY++){
                    for(var posX = -webDist; posX <= webDist; posX++){
                        var compoundIndex = (posY+webDist)*webWidth + (posX+webDist);
                        var testTile = viewGrid[compoundIndex];
                        var testBlock = ownLevel.getTileContents(self.x+posX, self.y+posY, true);
                        if(Math.random() < 3/4 && testTile && !testTile.dense && !testBlock){
                            var webTrap = modelLibrary.getModel('trap', 'web');
                            Object.instantiate(webTrap).place(self.x+posX, self.y+posY, self.levelId);
                        }
                    }
                }
            }, 1);
...
modelLibrary.registerModel('enemy', Object.create(spiderPrototype, {
    // Id:
    generationId: {value: 'tarantula', writable: true},
